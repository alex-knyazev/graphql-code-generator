...
class TsVisitor extends visitorPluginCommon.BaseTypesVisitor {
  constructor(schema, pluginConfig, additionalConfig = {}) {
    super(schema, pluginConfig, {
      ...
      avoidOptionals: visitorPluginCommon.normalizeAvoidOptionals(
        visitorPluginCommon.getConfigValue(pluginConfig.avoidOptionals, false)
      ),
      maybeValue: visitorPluginCommon.getConfigValue(
        pluginConfig.maybeValue,
        "T | null"
      ),
      fieldWrapperValue: visitorPluginCommon.getConfigValue(
        pluginConfig.fieldWrapperValue,
        "T"
      ),
  ...
  getMaybeValue() {
    return `${this.config.noExport ? "" : "export "}type Maybe<T> = ${
      this.config.maybeValue
    };`;
  }
  getFieldWrapperValue() {
    return `${this.config.noExport ? "" : "export "}type FieldWrapper<T> = ${
  ...
  wrapWithListType(str) {
    const arrayType = this.config.immutableTypes ? "ReadonlyArray" : "Array";
    return `${arrayType}<${str}>`;
    // return this.config.wrapFieldDefinitions ? `${arrayType}<FieldWrapper<${str}>>` : `${arrayType}<${str}>`;
  }
  NonNullType(node) {
  ...
  FieldDefinition(node, key, parent) {
    ...
    if (!this.config.wrapFieldDefinitions) {
      ...
      // if field is a list - we should add field resolver in list elem
      if (node.type.startsWith("Array")) {
        typeString = node.type.replace("Array", "Array<FieldWrapper") + ">";
      } else if (node.type.startsWith("Maybe<Array")) {
        typeString =
          node.type.replace("Maybe<Array", "Maybe<Array<FieldWrapper") + ">";
      } else {
            ...
            node.values
              .map((enumOption) => {
                ...
                if (
                  ...
                  enumValue = this.config.enumValues[enumName].mappedValues[
                    enumValue
                  ];
                }
                return (
